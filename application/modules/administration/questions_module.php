...
// Questions class
class Questions_Module extends CI_Module {
	...
	public function index($temp_method = "", $letter = "", $temp_string = "", $string = "", $temp_page = "", $page = "") {
		...
		} else if ($this->uri->segment(4) == 'category') {
			$where = array( 'category_id' => $this->uri->segment(5) );
		} else {
			$where ='';
		}
		$limit= array();
		...
		if ($this->uri->segment(4) <> 'member' && $this->uri->segment(4) <> 'category') {
		...
		// Get questions
		$vars['items'] = $this->questions->get_questions_admin($where, $limit);
		...
		// Display template
		$this->view('administration/questions/browse', $vars);
	}
	...
	public function edit(){
		...
		if(strtolower($_SERVER['REQUEST_METHOD']) == "post"){
			...
			if(empty($vars['errors'])){
				...
				redirect("/administration/questions/");
			}
			
		}
		$question = $this->questions->get_questions(array("q.id" => $idquestion));
		// Display template
		$this->view('administration/questions/form', 
			array(
				"question" 		=>	$question[0],
				"categories"	=>  $this->questions->get_categories(),
				"action"		=>	"edit",
				"errors"		=>	$errors,
				"tinyMCE"		=>	TRUE
			)
	...
	public function active(){
		...
		// Update question
		$this->questions->save_question(array("status" => $status), $this->input->post("id"));
	...
	public function delete(){
		...
		// Remove record
		$this->questions->delete_question($idquestion);
