...
<script type="text/javascript">var fromPage = 'projects/delete_update';</script>

<div class="mainForm"><fieldset>
	<dl>
		<form id="search-form" method="post" action="/administration/projects/updates/">
			<dd>
...
<?php if(!empty($items)){ ?>
	<tbody>
		<?php foreach ($items as $comment){ ?>
				<tr id="<?php echo $comment->idupdate ?>">
					...
					<td>
						...
						<?php if($comment->comments_count > 0){ ?>
							<a href="/administration/projects/updates/string/all/idproject/<?php echo $comment->idproject?>/">(<?php echo $comment->comments_count?>)</a>
						<?php } else { ?>
							(0)
						<?php } ?>
					</td>
					...
					<td class="icons" style="width: 90px;">
						<a href="<?php echo site_url('administration/projects/edit_update/'.$comment->idupdate); ?>" class="edit" title="Edit">Edit</a>
