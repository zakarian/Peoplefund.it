	...
	$.fn.lightBox = function(settings) {
		...
		function _set_image_to_view() { // show the loading
			...
			objImagePreloader.onload = function() {
				...
				var y = arrPageSizes[3] - 190;

				if (objImagePreloader.width > x) {
					objImagePreloader.height = objImagePreloader.height * (x / objImagePreloader.width); 
					objImagePreloader.width = x; 
					if (objImagePreloader.height > y) { 
						objImagePreloader.width = objImagePreloader.width * (y / objImagePreloader.height); 
						objImagePreloader.height = y; 
					}
				} else if (objImagePreloader.height > y) { 
					objImagePreloader.width = objImagePreloader.width * (y / objImagePreloader.height); 
					objImagePreloader.height = y; 
					if (objImagePreloader.width > x) { 
						objImagePreloader.height = objImagePreloader.height * (x / objImagePreloader.width); 
		...
		function _show_image_data() {
			...
			$('#lightbox-image-details-caption').html('<a href="/administration/assets/download/' + Base64.encode( settings.imageArray[settings.activeImage][0] ) + '/">Download</a>').show();
