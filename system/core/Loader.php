...
class CI_Loader {
	...
	var $_ci_modules		= array(); //Customly added modules
	...
	function model($model, $name = '', $db_conn = FALSE)
	{
		if (is_array($model))
		{
			// changes by lanzz: allow specifying aliases by using associative array elements
		...
		foreach ($this->_ci_model_paths as $mod_path)
		{
			...
			// changes by lanzz: record the model as loaded BEFORE instantiating it
			// this avoids cycles in model constructors loading each other
	 ...
	 * Module Loader
	 *
	 * This function lets users load and instantiate different modules - custm classes.
	 *
	 * @access	public
	 ...
	 * @return	void
	 */
	function module($name = 'index', $parameters = array())
	{
		$path = '';
        $module = $name;
        ...
        $module_class_name = $module.'_Module';
		$CI =& get_instance();
		if (isset($CI->$module_class_name))
		{
			show_error('The module name you are loading is the name of a resource that is already being used: '.$module_class_name);
		}
		...
		{
			if ( ! file_exists($mod_path.'modules/'.$path.$module.'_module'.EXT))
			{
				continue;
			}
			...
			require_once($mod_path.'modules/'.$path.$module.'_module'.EXT);

			$module = ucfirst($module);
            $module = $module.'_Module';

			$CI->$module = new $module($parameters);
